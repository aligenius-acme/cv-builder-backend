...
function extractResumeDataRuleBased(rawText) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const isLikelyHeader = (cleanLine.length < 60 &&
...
function extractCertificationsFromText(lines) {
...
function formatWithBullets(lines) {
    return lines
        .map(line => {
    ...
    const saveExperience = () => {
        if (current && (current.title || current.company)) {
            ...
            if (!current.description)
                current.description = [];
            if (!current.title)
                current.title = current.company || 'Unknown Position';
            if (!current.company)
                current.company = '';
...
function parseProjects(content) {
    ...
    for (let i = 0; i < content.length; i++) {
        ...
        const isLikelyHeader = !isBulletPoint && line.length < 100 && (hasDate ||
