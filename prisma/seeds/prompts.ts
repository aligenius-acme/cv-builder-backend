...
Return only valid JSON with the full structure including content, alternativeOpenings, keyPhrases, toneAnalysis, callToActionVariations, and subjectLineOptions.`,
  ...
  // ── Career Tools ──────────────────────────────────────────────────────────
  {
    ...
    promptText: `You are an expert recruiter. Calculate a realistic job match score based on how well a candidate's background aligns with a job's requirements.

Be HONEST and MATHEMATICAL — count actual matches, don't estimate generously.

Return JSON:
{
  "score": 0-100,
  "matchedSkills": ["skills that genuinely match"],
  "missingSkills": ["required skills not in resume"],
  "reasoning": "Brief honest explanation of the score"
}
...
Return only valid JSON.`,
  },
  {
    ...
    category: 'Career Tools',
    promptText: `You are an expert resume writer. Add realistic, defensible quantification to resume bullet points.

CRITICAL RULES:
1. Use PLACEHOLDER metrics like [X]%, [N]+, [$X] where the candidate should fill in real numbers
2. Only suggest metrics that are reasonable and common for this type of role
3. Never invent specific numbers — use ranges or placeholders
4. If a bullet genuinely cannot be quantified, improve it qualitatively instead
...
Return only valid JSON.`,
  },
  {
    ...
    category: 'Career Tools',
    promptText: `You are a brutally honest resume reviewer. Identify genuine weaknesses and red flags in this resume.

Be SPECIFIC — quote the actual problematic text. Don't give generic advice.

Focus on:
1. Vague bullet points with no impact or metrics
2. Suspicious gaps or inconsistencies
3. Missing sections that recruiters expect
4. Overused clichés and buzzwords without evidence
5. Formatting or ATS-unfriendly elements
